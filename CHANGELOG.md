...
## 0.1.2 (2026-09-22) — baseline docxtpl 0.20.1

- Packaging only: the PyPI long description is now `README.pypi.md`,
  generated by `scripts/pypi_readme.py` from the English and Simplified
  Chinese READMEs (PyPI renders one file and cannot follow
  repository-relative links, so the language switch pointed nowhere). CI
  fails when the generated file is stale. No code changes.
